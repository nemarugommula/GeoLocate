  ...
  function showResultInPanel(result) {
    ...
    area.querySelectorAll(".scenepoint-fb-btn").forEach((btn) => {
      btn.addEventListener("click", () => {
        const container = btn.closest(".scenepoint-sp-feedback");
        if (btn.dataset.vote === "up") {
          ...
          document.getElementById("scenepoint-submit-correction").addEventListener("click", () => {
            const correction = document.getElementById("scenepoint-correction").value;
            submitFeedbackDirect(btn.dataset.id, "down");
          ...
          document.getElementById("scenepoint-cancel-correction").addEventListener("click", () => {
            container.innerHTML = `
              <span>Was this correct?</span>
              <button class="scenepoint-fb-btn" data-vote="up" data-id="${btn.dataset.id}">👍 Yes</button>
              <button class="scenepoint-fb-btn" data-vote="down" data-id="${btn.dataset.id}">👎 No</button>`;
            // Re-attach listeners (recursive call pattern avoided — just submit directly)
            container.querySelectorAll(".scenepoint-fb-btn").forEach(b => b.addEventListener("click", () => {
              if (b.dataset.vote === "up") {
                submitFeedbackDirect(b.dataset.id, "up");
                container.innerHTML = `<span class="scenepoint-fb-thanks">Thanks! Glad it was correct.</span>`;
              }
            }));
          });
